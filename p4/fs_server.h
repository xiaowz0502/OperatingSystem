 ...
 */
struct fs_direntry {
    char name[FS_MAXFILENAME + 1];         // name of this file or directory            60 bytes
    uint32_t inode_block;                  // disk block that stores the inode          4 bytes
                                           // for this file or directory (0 if
                                           // this direntry is unused)
};

struct fs_inode {
    char type;                             // file ('f') or directory ('d') or          1 byte
                                           // unused ('u')                              
    char owner[FS_MAXUSERNAME + 1];        // owner of this file or directory           11 bytes
    uint32_t size;                         // size of this file or directory            4 bytes
                                           // in blocks
    uint32_t blocks[FS_MAXFILEBLOCKS];     // array of data blocks for this             4 bytes per block
                                           // file or directory                         124 blocks
};
