...
int main(int argc, char *argv[])
{
    ...
    status = fs_writeblock("user1", "/dir/file", 001, writedata);
    assert(!status);

    status = fs_readblock("user1", "/dir/file", 0, readdata);
    assert(!status);

    cout << "output: " << readdata[7] << endl;
    ...
    status = fs_create("user1", "/dir/lay1", 'f');
    assert(status);
    ...
    status = fs_create("user1", "/dir/lay1/lay2", 'f');
    assert(status);
    ...
    status = fs_create("user1", "/dir/lay1/lay2/lay3", 'f');
    assert(status);
